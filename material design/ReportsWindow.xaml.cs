...
namespace material_design
{
    public partial class ReportsWindow : Window
    {
        ...
        public ReportsWindow()
        {
            ...
            var regularClientRepo = new Repository<Regular_Clients>(_context);
            var postRepo = new Repository<Post>(_context);
            _reportService = new ReportService(orderRepo, orderDetailsRepo, menuRepo, categoryRepo,
                employeeRepo, clientRepo, regularClientRepo, postRepo);
            ...
            try
            {
                switch (reportType)
                {
                    case "Отчет по продажам за период":
                        dgReport.ItemsSource = _reportService.GetSalesReport(startDate, endDate);
                        break;
                    case "Эффективность сотрудников":
                        dgReport.ItemsSource = _reportService.GetEmployeeReport(startDate, endDate);
                        break;
                    case "Анализ клиентской базы":
                        dgReport.ItemsSource = _reportService.GetClientReport();
                        break;
                    case "Популярность меню":
                        dgReport.ItemsSource = _reportService.GetMenuReport(startDate, endDate);
                        break;
                    case "Финансовый отчет":
                        dgReport.ItemsSource = _reportService.GetFinancialReport(startDate, endDate);
                        break;
                }
            }
            catch (Exception ex)
